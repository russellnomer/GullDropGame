...
export class GullDropGame {
  ...
  private update(dt: number) {
    ...
    if (st.phase === "playing") {
      ...
      this.courtship.update(dt, {
        ...
        threatNear: (kind, x, y, z) => {
          if (kind === "squirrel") {
            return this.strayCats.list.some(
              (cat) => cat.alive && Math.hypot(cat.x - x, cat.y - y, cat.z - z) < 8,
            );
          }
          return this.sharks.sharks.some(
            (shark) =>
              shark.active &&
              shark.state !== "cooldown" &&
              Math.hypot(shark.x - x, shark.y - y, shark.z - z) < 10,
          );
        },
      });
    }
  ...
  private shoot() {
    ...
    slot.age = 0;
    slot.trailT = 0;
    slot.x = origin.x + rear.x * 1.55 + down.x * 0.62;
    slot.y = origin.y + rear.y * 1.55 + down.y * 0.62;
    slot.z = origin.z + rear.z * 1.55 + down.z * 0.62;
    slot.vx = this._look.x * 13 + this.vx * 0.55;
    slot.vy = Math.min(-6.5, this._look.y * 8 - 5.5 + this.vy * 0.35);
    slot.vz = this._look.z * 13 + this.vz * 0.55;
  ...
  private updateMafiaMode(dt: number, mx: number, my: number, throttle: number, strafe: number) {
    ...
    if (trib) {
      prompt = `Press E to collect ${trib.amount} respect`;
      if (interactBtn) {
        track("tribute_collect", { amount: trib.amount });
