...
import { BRIGHTON_PARK } from "./brighton-park";
...
export class DogWalkers {
  ...
  spawnAll() {
    for (let i = 0; i < 4; i++) this.spawn("boardwalk");
    for (let i = 0; i < 2; i++) this.spawn("beach");
    for (let i = 0; i < 4; i++) this.spawn("park");
  }
  ...
  private resetPair(p: DogWalkerPair) {
    ...
    p.x = p.zone === "beach" ? -20 - this.random() * 20 : p.zone === "park" ? 62 + this.random() * 34 : -6 + this.random() * 12;
    p.y = p.zone === "beach" ? SAND_Y : p.zone === "park" ? BRIGHTON_PARK.y : BOARDWALK.y;
    p.z = p.zone === "park" ? 157 + this.random() * 42 : -200 + this.random() * 400;
  ...
  private spawn(zone: "boardwalk" | "beach" | "park") {
    ...
    for (const p of this.pairs) {
      ...
      if (p.hitPerson || p.hitDog) {
        ...
        if (p.zone === "beach") {
          ...
          if (p.x < 60 || p.x > 98) p.yaw += Math.PI;
        } else {
          if (p.x < -6.5 || p.x > 6.5) p.yaw += Math.PI;
        }
        if (p.z < (p.zone === "park" ? 154 : -210) || p.z > (p.zone === "park" ? 202 : 210)) p.yaw += Math.PI;
