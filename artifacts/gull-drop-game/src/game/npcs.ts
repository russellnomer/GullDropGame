...
export class Crowd {
  ...
  spawnAll(benches: THREE.Vector3[], photo: THREE.Vector3, kiosk: THREE.Vector3, rooftops: { kind: "vip" | "pitboss"; x: number; y: number; z: number }[] = []) {
    const pack = [
      [0.8, 8],
      [-1.6, 6],
      [2.2, 4],
    ...
    for (const target of rooftops) this.spawnRooftop(target.kind, target.x, target.y, target.z);
  }

  private spawnRooftop(kind: "vip" | "pitboss", x: number, y: number, z: number) {
    const n = this.spawn(kind, x, z);
    n.y = y;
    n.home = new THREE.Vector3(x, y, z);
    n.speed = kind === "pitboss" ? 0.55 : 0.75;
    n.group.position.set(x, y, z);
    n.group.scale.setScalar(kind === "pitboss" ? 1.16 : 1.08);
  }
  ...
  private spawnBeach() {
    ...
    for (let i = 0; i < 10; i++) this.spawn("tanner", -20 - this.random() * 18, -160 + i * 32 + this.random() * 10);
    for (let i = 0; i < 8; i++) this.spawn("swimmer", -70 + this.random() * 18, -140 + i * 38 + this.random() * 12);
    for (let i = 0; i < 6; i++) this.spawn("volleyball", -34 + this.random() * 16, -120 + i * 42);
  }
  ...
  spawn(kind: NpcKind, x?: number, z?: number, starRole?: StarRole, mount?: THREE.Object3D): Npc {
    ...
    if (kind === "vip" || kind === "pitboss") {
      const jewel = new THREE.Mesh(
        new THREE.SphereGeometry(kind === "pitboss" ? 0.1 : 0.075, 10, 8),
        std(kind === "pitboss" ? 0xc45c4a : 0xc9a227, { emissive: kind === "pitboss" ? 0xc45c4a : 0xc9a227, emissiveIntensity: 1.1 }),
      );
      jewel.position.y = 2.25;
      group.add(jewel);
    }
    ...
    const n: Npc = {
      ...
      alive: true,
      hit: false,
      x: x ?? (surf ? -70 + this.random() * 20 : sand || kind === "lifeguard" ? -36 + this.random() * 18 : -5 + this.random() * 10),
      y: kind === "lifeguard" ? SAND_Y + 3.15 : surf ? 0.15 : sand ? SAND_Y : BOARDWALK.y,
      z: z ?? (sand || surf || kind === "lifeguard" ? -180 + this.random() * 360 : this.random() < 0.5 ? -50 + this.random() * 80 : -180 + this.random() * 360),
      yaw: this.random() * Math.PI * 2,
      speed: 1.1 + this.random() * 0.9,
      walk: this.random() * 10,
      panic: 0,
      eat: kind === "custard" ? 0.4 + this.random() * 0.5 : 0,
      partner: null,
      group,
      cone,
      item,
      react: (this.random() * 3) | 0,
      glance: 0,
      splat: null,
      bubble: null,
      mount: mount ?? null,
  ...
  respawn(n: Npc, kind: NpcKind) {
    ...
    n.panic = 0;
    n.glance = 0;
    n.react = (this.random() * 3) | 0;
    n.eat = kind === "custard" ? 0.5 : 0;
    if ((kind === "vip" || kind === "pitboss") && n.home) {
      n.x = n.home.x;
      n.y = n.home.y;
      n.z = n.home.z;
    ...
    if (kind === "vendor") {
      n.x = -5 + this.random() * 10;
      n.y = BOARDWALK.y;
      n.z = -180 + this.random() * 360;
    } else if (kind === "swimmer") {
      n.x = -70 + this.random() * 20;
      n.y = 0.15;
      n.z = -180 + this.random() * 360;
    } else if (kind === "tanner" || kind === "volleyball") {
      n.x = -36 + this.random() * 18;
      n.y = SAND_Y;
      n.z = -180 + this.random() * 360;
    } else if (kind === "lifeguard") {
      n.x = -16.5;
      n.y = SAND_Y + 3.15;
      n.z = [-90, 8, 118][(this.random() * 3) | 0];
    } else {
      n.x = -5 + this.random() * 10;
      n.y = BOARDWALK.y;
      n.z = -180 + this.random() * 360;
  ...
  /** Restores every score-relevant pedestrian from the current seeded stream. */
  reset() {
    for (const n of this.npcs) {
      if (n.mount) {
        this.respawn(n, "rider");
        continue;
      }
      n.yaw = this.random() * Math.PI * 2;
      n.speed = 1.1 + this.random() * 0.9;
      n.walk = this.random() * 10;
      this.respawn(n, n.initialKind);
    }
  }
    ...
    for (const n of this.npcs) {
      ...
      if (n.hit) {
        ...
        if (p > 2.6) {
          ...
          this.respawn(n, n.kind === "star" || n.kind === "cop" || n.kind === "vendor" || n.kind === "rider" || n.kind === "vip" || n.kind === "pitboss" || beachKind(n.kind) ? n.kind : "tourist");
        }
        continue;
      }
      if (n.glance > 0) {
      ...
      if ((n.kind === "vip" || n.kind === "pitboss") && n.home) {
        n.yaw += dt * (n.kind === "pitboss" ? 0.5 : -0.35);
        n.x = n.home.x + Math.sin(n.walk * 0.25) * (n.kind === "pitboss" ? 0.65 : 1.05);
        n.z = n.home.z + Math.cos(n.walk * 0.25) * 0.55;
        n.group.position.set(n.x, n.y, n.z);
        n.group.rotation.set(0, n.yaw, 0);
        continue;
      }
  ...
  hitTest(x: number, y: number, z: number, r: number): { npc: Npc; cone: boolean } | null {
    for (const n of this.npcs) {
      ...
      const dy = y - bodyY;
        const rad = n.kind === "tanner" || n.kind === "swimmer" || n.kind === "rider" || n.kind === "vendor" || n.kind === "pitboss" ? 1.3 : 1.05;
