...
export class CourtshipVignette {
  ...
  update(dt: number, time: number | { reducedMotion?: boolean }) {
    const reducedMotion = typeof time === "object" ? (time.reducedMotion ?? false) : false;
    ...
    if (this.state === "approach") {
      ...
      const bob = reducedMotion ? 0 : Math.abs(Math.sin(this.timer * 10)) * 0.1;
      ...
      const jump = reducedMotion ? 0 : Math.abs(Math.sin(this.timer * 15)) * 0.4;
      ...
      const escapeBob = reducedMotion ? 0 : Math.abs(Math.sin(this.timer * 25)) * 0.3;
      this.actorA.position.y = this.cy + escapeBob;
      this.actorB.position.y = this.cy + escapeBob;
...
export class CourtshipManager {
  ...
  update(dt: number, options: number | { reducedMotion?: boolean }) {
