...
export class ParticlePool {
  ...
  drip(x: number, y: number, z: number, vx: number, vy: number, vz: number) {
    ...
    p.y = y;
    p.z = z;
    p.vx = vx * 0.08 + (Math.random() - 0.5) * 0.35;
    p.vy = Math.min(-0.8, vy * 0.08) - Math.random() * 0.8;
    p.vz = vz * 0.08 + (Math.random() - 0.5) * 0.35;
    p.s = 0.42 + Math.random() * 0.28;
    this.mesh.setColorAt(this.items.indexOf(p), _c.setHex(0xf4f1ea));
    if (this.mesh.instanceColor) this.mesh.instanceColor.needsUpdate = true;
  }
