...
export class TributeManager {
  ...
  constructor() {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      this.group.add(mesh);
      this.ops.push({
        id: i, x: 0, y: 0, z: 0, amount: 0, active: false, mesh, respawnTime: 0, kind: "generic"
      });
    }
  }
  ...
  spawnAll() {
    ...
    // Boardwalk locations
    this.spawn(8, -88, 200, "taffy-shop");
  ...
  /**
   * spawn — Activates a pooled protection target at a world location.
   *
   * @param x - Horizontal world coordinate.
   * @param z - Longitudinal world coordinate.
   * @param amount - Base carried-tribute reward.
   * @param kind - Encounter identity used for location-specific dialogue.
   * @returns Nothing.
   *
   * Business context: named shops can deliver bespoke extortion encounters
   * without adding more meshes or per-frame allocations.
   */
  spawn(x: number, z: number, amount: number, kind: TributeOp["kind"] = "generic"): void {
