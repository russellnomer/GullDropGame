...
router.get("/gull-drop/fulfillment", async (req, res): Promise<void> => {
  ...
  const session = (await stripeResponse.json()) as {
    ...
    metadata?: { offer?: string; source?: string; creator?: string };
    amount_total?: number;
    currency?: string;
    mode?: string;
  ...
  if (entitlementIsActive && userId) {
    const [current] = await db.select().from(gullDropProfiles).where(eq(gullDropProfiles.userId, userId)).limit(1);
    const entitlements = Array.from(new Set([...(current?.entitlements ?? []), verifiedOffer]));
    await db.insert(gullDropProfiles)
