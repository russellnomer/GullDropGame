 ...
 */
import { db, gullDropEntitlements, gullDropProfiles, gullDropPurchases, gullDropStripeEvents } from "@workspace/db";
  ...
  return db.transaction(async (tx) => {
    ...
    try {
      ...
      if (kind === "checkout") {
        ...
        if (purchase.userId) {
          await tx.execute(sql`select pg_advisory_xact_lock(hashtext(${"gull-drop-account:" + purchase.userId}))`);
        }
        ...
        await tx.insert(gullDropPurchases).values({
          ...
          set: {
            ...
            paymentIntentId: purchase.paymentIntentId,
            chargeId: purchase.chargeId,
            userId: sql`coalesce(excluded.user_id, ${gullDropPurchases.userId})`,
            updatedAt: new Date(),
          },
        });
        ...
        const purchases = await tx.select({
          ...
          userId: gullDropPurchases.userId,
        })
        ...
        for (const purchase of purchases) {
          ...
          if (purchase.userId) {
            const otherAccountPurchases = await tx.select({ id: gullDropPurchases.checkoutSessionId })
              .from(gullDropPurchases)
              .where(and(
                eq(gullDropPurchases.userId, purchase.userId),
                eq(gullDropPurchases.offer, purchase.offer),
                eq(gullDropPurchases.status, "paid"),
              ))
              .limit(1);
            if (otherAccountPurchases.length === 0) {
              await tx.update(gullDropProfiles)
                .set({
                  entitlements: sql`array_remove(${gullDropProfiles.entitlements}, ${purchase.offer})`,
                  updatedAt: new Date(),
                })
                .where(eq(gullDropProfiles.userId, purchase.userId));
            }
          }
        }
      }
